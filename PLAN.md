...
| §4.4 | TUI framework: `ink` | **Adopted 2026-08-01**, when the TUI grew to six panes and a form. Reached through a dynamic import behind `splitting: true`, so `qv inbox` never loads React — asserted by `test/unit/bundle.test.ts` and measured at 382 ms vs a 373 ms baseline. The interim hand-rolled renderer is gone. |
| §4.4 | TUI spawns `qv tx approve --json` and parses the result | **All three stdio streams are inherited instead.** Piping stdout while inheriting stdin produces the worst case: the child asks its §7 confirmation question where the user cannot see it and waits for an answer. Inheriting means the reviewer sees exactly what a one-shot user sees, and the exit code carries the result. |
